...
- [x] **Theme parity** — cyberpunk palette hand-converted from web's oklch values to sRGB hex (Flutter has no oklch), plus the font chosen in Phase 5.5 bundled as a font asset (JetBrains Mono until then). Flat colours only; glow, scanline, animation and sound are Phase 5.5, not part of the basic loop. _Done with the Phase 5.5 mobile work: Phase 5.5 tokens in `lib/core/theme.dart`, Oxanium + Orbitron bundled._
...
- [ ] **`flutter build apk` in `mobile.yml`** — add once a native plugin lands, so plugin/Gradle breakage is caught in CI and not on a dev machine. **The trigger has fired:** `audioplayers` (Phase 5.5 mobile) is the first native plugin. Not added yet, because the build couldn't be verified where that work was done (no Android SDK). Add the step after confirming `flutter build apk --debug` passes locally or on the runner.
...
Design decided 2026-09-25: `docs/superpowers/specs/2026-09-25-phase5.5-cyberpunk-polish-design.md` (Oxanium + Orbitron, signal-yellow `#FCEE0A` as the event/action accent, one clipped-ring effect primitive; interactive POC at <https://claude.ai/artifact/32CancxRN7HWtmZ2AeCpJt>). **Mobile done, web pending** for the first three bullets below.

- [ ] **Cyberpunk font.** _Mobile done, web pending: Oxanium (UI, digits) + Orbitron (logo, headings), per the spec._ Replace JetBrains Mono with a font that reads as cyberpunk. Pick it in the proof of concept. The digits must stay legible at cell size, so a likely split is a display face for the logo and headings, and a techy mono or semi-mono face for UI and grid digits. Candidates (all SIL OFL, on Google Fonts, available as `@fontsource/*` for web and bundleable as Flutter assets): Orbitron or Audiowide (display only, too wide for body text), Chakra Petch, Oxanium, Share Tech Mono. Use tabular or monospaced digits so the timer doesn't jitter.
- [ ] **Given vs. player digits, clearly distinct.** _Mobile done, web pending: spec section 3._ Today web makes givens `neutral-200` semibold and entries `neutral-400` medium: two greys that are hard to tell apart, and the same-value highlight recolours both to `accent-300`, which erases the difference. Mobile uses white bold vs. `accent-300`. Target, identical on both clients:
...
- [ ] **Unit-complete effects.** _Mobile done, web pending: clipped ring, yellow bracket flash, derived scanline hatch, event line, SECTORS counter, chirp and haptic; no effect on the winning move._ When a row, column or 3×3 box becomes full with no conflicts:
...
- [ ] **Pending: celebrate only correct units** once validation is decided (Phase 2.7 "Per-move mistake checking"). Until then "complete" = full with no peer conflicts, worded "complete"/"secured", never "correct". Only `completeUnits` changes on both clients (spec section 8, Q4).
- [ ] **Flutter animations and sounds (mobile only).** Port the Phase 2.6 set:
  ...
  - _Mobile progress: digit pop-in, the unit-complete chirp (`tool/render_chirps.dart` → `assets/sfx/`, played through `audioplayers`), the unit-complete haptic, and an in-memory effects/sound/haptics panel in the pause overlay. The rest below is still open._
