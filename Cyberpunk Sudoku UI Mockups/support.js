...
(() => {
  ...
  function hostPositionStyle(style) {
    const all =
      typeof style === "string" ? cssToObj(style) : style != null && typeof style === "object" ? style : null;
  ...
  function isDeckMountTag(el) {
    ...
    return (
      el.localName === "x-import" && (el.getAttribute("component-from-global-scope") || "") === "deck-stage"
    );
  }
    ...
    const nextSlideId = new Array(pairs.length);
    {
      ...
      for (let j = pairs.length - 1; j >= 0; j--) {
        ...
        if (n.nodeType === Node.ELEMENT_NODE) {
          const t = n.localName;
          upcoming =
            !DECK_AUX_RE.test(t) && !DECK_CONTROL_FLOW_RE.test(t) ? n.getAttribute("data-om-slide-id") : null;
        }
          ...
          if (v === void 0) {
            if (!ctx?.__streamingNow) {
              if (document.body?.hasAttribute("data-dc-editor-on")) {
                return h("span", { key: i, className: "sc-interp sc-unresolved" }, "{{ " + p.trim() + " }}");
              }
      ...
      if (!Array.isArray(list)) {
        if (!ctx?.__streamingNow) {
          if (list !== void 0 && list !== null) {
            warnUnresolved(ctx, 'sc-for list="' + listSrc + '" is not an array (' + typeof list + ")");
          }
          list = [];
        } else {
  ...
  function walkXImport(el, host) {
    ...
    const exportNameGet = compileAttr(el.getAttribute("component") || el.getAttribute("name") || "");
    const fromRaw =
  ...
  function Placeholder({ name, hintSize, streaming, error }) {
    ...
    return h(
      ...
      error ? h("div", { className: "sc-placeholder-error" }, (name ? name + ": " : "") + error) : null
    );
  }
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      ...
      componentDidCatch(e, info) {
        console.error("[dc-runtime] render error in <" + this.__name + ">:", e, info?.componentStack || "");
      }
      ...
      render() {
        ...
        if (!r.tpl) {
          return h("div", hostBase, h(Placeholder, { name: this.__name, hintSize: this.props.__hintSize }));
        }
        ...
        try {
          ...
          console.error(e);
          renderErr =
            this.__name + ".renderVals(): " + (e instanceof Error && e.message ? e.message : String(e));
        }
        ...
        return h(
          ...
          renderErr && h("div", { className: "sc-logic-error", "data-omelette-chrome": "" }, renderErr),
  ...
  function isElementClass(g) {
    try {
      return (
        typeof g === "function" && typeof HTMLElement !== "undefined" && g.prototype instanceof HTMLElement
      );
    } catch {
      return false;
    }
      ...
      return (_vals, ctx) => {
        ...
        for (let i = 0; i < raw.length; i++) {
          ...
          if (tag === "SCRIPT") {
            ...
            if (mayBePartial) continue;
            const key =
              tag + "|" + (child.getAttribute("href") || child.getAttribute("src") || child.outerHTML);
            if (mounted.has(key)) continue;
            mounted.add(key);
            if (tag === "LINK") {
              ...
              const res = window.__resources;
              const pre =
                res && rel.includes("stylesheet") && !rel.includes("alternate") ? res[href] : void 0;
              ...
              if (blob) {
                ...
                if (child.getAttribute("title")) el.setAttribute("title", child.getAttribute("title"));
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      (blob
        ...
        .catch((e) => console.error('[dc-runtime] sibling fetch for "' + name + '" threw:', url, e));
    }
    let rootName = null;
    ...
    function updateJs(name, src) {
      ...
      try {
        ...
        if (typeof Cls !== "function") {
          r.logicError =
            name + ".dc.html: <script data-dc-script> must define `class Component extends DCLogic`";
        } else {
          r.logicError = null;
          r.Logic = Cls;
        }
